...
public class QuanLySinhVien {
	...
	public static void main(String[] args) {
	 		ArrayList<SinhVienFpt> DanhSach = new ArrayList<SinhVienFpt>();
	 		while (true) {
	 			System.out.println("-------");
				...
				System.out.println("5. Xuat ra sinh vien theo nganh ");
	 			...
	 			if (choice == 2) {
	 				...
	 				for (int i = 0; i < DanhSach.size(); i++) {
	 					if (DanhSach.get(i).getHocLuc().equals("Gioi")) {
	 						DanhSach.get(i).xuat();
	 					}
	 				}
	 			} else if (choice == 4) {
	 				Collections.sort(DanhSach, new Comparator<SinhVienFpt>() {
				...
				System.out.println("Sinh vien thuoc nganh IT ");
					for (SinhVienFpt svFPT : DanhSach) {
